...
Scaling:
```{r, cache=TRUE}
# Intentionally slow version for teaching
system.time(e.standardize.slow <-
                t(apply(e, 1, function(x)
                    x - mean(x))))
# A clearer but still slow alternative to apply()
system.time(e.standardize.sweep <- sweep(e, 1, rowMeans(e)))
system.time(e.standardize.fast <- t(scale(t(e), scale = FALSE)))
system.time(e.standardize.fastest <- e - rowMeans(e))
all.equal(e.standardize.slow, e.standardize.fastest, check.attributes = FALSE)
```

**Note:** `e - rowMeans(e)` is the most efficient method because it leverages R's column-major matrix storage and vectorized subtraction, avoiding the overhead of `apply`, `sweep`, or multiple transpositions.
...
library(Biobase)
library(genefilter)
library(GSE5859) ## BiocManager::install("genomicsclass/GSE5859")
data(GSE5859)
